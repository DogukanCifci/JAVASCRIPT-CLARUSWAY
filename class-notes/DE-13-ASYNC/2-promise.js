...
//API, Database vb veri cekmek icin hazir suan
const myPromise1 = new Promise((resolve, reject) => {
  const person = { name: "Ali", surname: "Gel" };

  resolve(person); //Basarili olursa person'u döndür
  reject(new Error("Promise basarisiz")); //Basarisiz olursa bunu döndür.
});

console.log(myPromise1);

//Daha sonra da then ile yazdirabiliriz.
myPromise1
  .then((response) => {
    console.log(response);
  })
  .catch((error) => console.log(error)); //exception as e gibi errorun ne oldugunu böyle yazdirabiliriz.
